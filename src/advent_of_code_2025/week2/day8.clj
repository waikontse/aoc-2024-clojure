...
(defn merge-circuits
  ...
  (let [_ (println "connecting from" from " to " to)
        from-connected-to (get seen from)
        to-connected-to (get seen to)
        _ (println "from-connected-to" "to-connected-to" from-connected-to to-connected-to)
  ...
  (loop [prioritized-dist (take max-pairs coll)
    ...
    (cond
      (empty? prioritized-dist) (do (println "Finishes connecting all pairs of" max-pairs "seen" (count seen))
                                    ;  (println "Seen: " seen)
                                    circuits)
       (and (= (count seen) nodes) (= 1 (count circuits))) (do (println "finished connecting all." last-connection) last-connection)
      ...
      (let [curr (first prioritized-dist)
            ...
            from-connected-to (get seen from)
            to-connected-to (get seen to)
            ;_ (clojure.pprint/pprint circuits)
...
(defn solve-part-1
  ...
  (let [raw-lines (clojure.string/split-lines input)
        ...
        ; _ (clojure.pprint/pprint (take 15 sorted-all-distances-combined))
